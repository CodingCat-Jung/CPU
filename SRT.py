# Shortest Remaining Time 스케줄링 (선점형)
# 최소 잔류 시간 우선 스케줄링
# 라운드 로빈 스케줄링을 사용하지만 남은 작업이 가장 적은 프로세스를 선택 - SJF + RR
from collections import  deque
from Process import *

# RR 스케줄링 코드에 남은 작업 시간 순으로 정렬하는 코드 추가
def SRT_run(processes, time_slice=2):
    ...
    while completed_processes < n:
        ...
        if ready_queue:
            ...
            # 정렬 코드 - SRT : 작업 시간이 짧은 순으로 정렬
            ready_queue = deque(sorted(list(ready_queue), key=lambda x: x.burst_time))
