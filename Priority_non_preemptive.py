...
def priority_non_preemptive_run(processes):
    print("비선점형 우선순위 Scheduling: ")
    current_time = 0
    ...
    # 1. 우선순위을 기준으로 프로세스 정렬
    priority_sorted = sorted(processes, key=lambda x:x.priority)
    ...
    # 그 다음 프로세스들은 우선순위 순서대로 진행
    for process in priority_sorted:
